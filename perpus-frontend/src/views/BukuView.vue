...
import { Eye, FolderPlus, Trash2 } from "lucide-vue-next";
...
<template>
  <div class="container flex-col p-4 mx-auto h-full">
    <div class="flex items-center mb-4 space-x-2">
      <Input v-model="cari" placeholder="Search books" class="max-w-sm" />
      ...
      <Table>
        ...
        <TableBody>
          <TableRow v-for="(item, index) in books" :key="item.isbn">
            {{ console.log(item) }}
            <TableCell class="font-medium">{{ nomor + index }}</TableCell>
            <TableCell class="truncate">{{ item.isbn }}</TableCell>
            <TableCell class="truncate">{{ item.judul }}</TableCell>
            <TableCell class="truncate">{{ item.pengarang }}</TableCell>
